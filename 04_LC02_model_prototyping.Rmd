...
huc2_basins <- read_sf(dsn="/Users/samsillen/Library/CloudStorage/OneDrive-UniversityofPittsburgh/nhd_data/HUC2_Merge", layer="HUC2_Merge") 
...
matchup <- read_feather("/Users/samsillen/Library/CloudStorage/OneDrive-UniversityofPittsburgh/OhioRiver_SR/Data/LC02_Matchups_new_coefs.feather")
...
outliers <- read_csv("/Users/samsillen/Library/CloudStorage/OneDrive-UniversityofPittsburgh/OhioRiver_SR/Models/Outlier detection/outlier_detection_old.csv") %>% #from outlier test
filter(count > 30)
...
# Save the results
write_feather(ffsResults, "C:/Users/samsi/OneDrive - University of Pittsburgh/OhioRiver_SR/Models/new_coefsffs_results__AOI_filter_outlier_raw_95_percentile.feather")
#
ffsResults %>%
  group_by(nvar) %>%
...
ffsResults <- read_feather("C:/Users/samsi/OneDrive - University of Pittsburgh/OhioRiver_SR/Models/new_coefsffs_results__AOI_filter_outlier_raw_95_percentile.feather")
...
saveRDS(final_model, file = "C:/Users/samsi/OneDrive - University of Pittsburgh/OhioRiver_SR/Models/new_coefs_final_model_95th_percentile.rDS")
...
final_model <- readRDS("C:/Users/samsi/OneDrive - University of Pittsburgh/OhioRiver_SR/Models/new_coefs_final_model_95th_percentile.rDS")
...
```{r}
# read riverSR 
riverSR <- read_feather("C:/Users/samsi/OneDrive - University of Pittsburgh/OhioRiver_SR/Data/LC02_polygons_COMID.feather")

huc2_basins <- read_sf(dsn="C:/Users/samsi/OneDrive - University of Pittsburgh/nhd_data/HUC2_Merge", layer="HUC2_Merge") 
st_transform(huc2_basins, crs ="+proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0")

Illinois <- read_sf("C:/Users/samsi/OneDrive - University of Pittsburgh/OhioRiver_SR/Data/illinois_huc.shp")

Ohio <- huc2_basins %>% filter(name == 'Ohio Region')

nhd <- read_sf("G:/My Drive/nhd_polys/nhd_COMID_poly.shp")

nhd <- nhd %>% st_transform(st_crs(Illinois))

Ohio_RiverSR <- st_join(nhd, Illinois) %>% drop_na(name)

sr <- riverSR %>% filter(COMID %in% Ohio_RiverSR$COMID)

sr$pred <- predict(final_model, sr[,ffs_features])
max(sr$pred)
# predicted value will be logged so you need to call exp 
sr <- sr %>% 
mutate(pred = exp(pred))

write_feather(sr, "C:/Users/samsi/OneDrive - University of Pittsburgh/OhioRiver_SR/Data/Illinoia_RiverSR_COMID.feather")

# prepare file with just Ohio River preds 

Ohio_IDS <- readRDS("C:/Users/samsi/OneDrive - University of Pittsburgh/dataViz_rShiny/Viz/OhioRiver_Viz/data/temp_v3.rDS") %>% select(ID)

 <- riverSR %>% filter(ID %in% Ohio_IDS$ID)

Ohio_RiverSR <- read_feather("C:/Users/samsi/OneDrive - University of Pittsburgh/OhioRiver_SR/Data/LC02_Ohio_RiverSR_preds.feather")


test <- Ohio_RiverSR %>% group_by(ID) %>% summarise(mean = max(pred))

test <- test %>% select(ID, mean)

riverSR_geom <- readRDS("C:/Users/samsi/OneDrive - University of Pittsburgh/dataViz_rShiny/Viz/OhioRiver_Viz/data/temp_v3.rDS")

RiverSR_geom <- riverSR_geom %>% 
select(ID, geometry)
...
RiverSR_geom <- RiverSR_geom %>% st_as_sf() %>% distinct(ID, .keep_all = TRUE)

RiverSR_geom <- readRDS( file = "C:/Users/samsi/Desktop/RiverSR_geom.rDS")

leaflet() %>%
        addProviderTiles(providers$CartoDB.Positron, group = 'Grey basemap') %>%
        addGlPolylines(data = RiverSR_geom, weight = 2.5,  opacity = 1,   color =  ~ pal4(mean),  popup = RiverSR_geom$ID, layerId =  RiverSR_geom$ID, group = "Reach-level mean") %>%
        setView(lat = 39.144348, lng = -84.098210,  zoom = 6) %>%
                addLegend("bottomleft", pal = pal4, values = RiverSR_geom$mean,
                  title = "Reach-level mean", group = 'Reach-level mean',
                  opacity = 1
        ) 
```