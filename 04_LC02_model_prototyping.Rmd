...
outliers <- read_csv("C:/Users/samsi/OneDrive - University of Pittsburgh/OhioRiver_SR/Models/.csv") # from outlier test 
...
aoa <- aoa(model = model, newdata = matchup_filter)

aoa_df <- as.data.frame(aoa$DI) %>% mutate(uniqueID = row_number())


new_data <- matchup_filter %>% 
mutate(uniqueID2 = row_number())


new_data <- new_data %>% 
filter(uniqueID2 %in% aoa_df$uniqueID)

new_data$pred <- predict(model, new_data[,ffs_features])

new_data$pred <- exp(new_data$pred)

ggplot(new_data , aes(x = (value), y = (pred))) + 
  geom_point() +
  xlim(0, 200) +
  ylim(0, 200) +
  geom_abline(slope=1, intercept = 0, color = 'black')+
