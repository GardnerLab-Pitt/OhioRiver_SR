...
#matchup$KAB <- 1.67-3.94*log(matchup$Blue)+3.78*log(matchup$Red)
#matchup$KRL <- (((matchup$Blue/matchup$Red)*matchup$Nir)-98)/0.75

```

# Settinb up base model (no ffs, hypertuning)
```{r}
...
features_1 <- df %>%
  select(NR:gn.gn, GCI, IRG, SABI, KIVU, GB, GNDVI, EVI) %>%
  names(.)
...
train_control_final <- caret::trainControl(
  ...
  verboseIter = T,
  allowParallel = TRUE,
  p = 0.8 #80% of the data is used to predict the other 20%
  )
...
output <- tibble(Predicted = pred, Actual = actual, uniqueID = uniqueID) %>%
  ...
  mutate(residual = Actual - Predicted,
         year = year(date),
         month = month(date),
         obs = ifelse(abs(residual) > 30, "bad", "good"))
